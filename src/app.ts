            ...
            if (row) {
                ...
                if (message_count < LIMIT) {
                    ctx.replyWithMarkdown(
                        `У вас осталось \`${LIMIT - message_count}\` сообщений.`
                    );
                } else {
                    ctx.replyWithMarkdown(
...
function processMessage(ctx: any, messageText: string, replyType: string) {    
    ...
    db.get(
        ...
        async (err, row) => {
            if (err) {
            	if (replyType === 'audio') audioReply(ctx, "Произошла ошибка"); else ctx.replyWithMarkdown(`Произошла ошибка: \`${err}\``);
                return;
            }
            ...
            if (message_count > LIMIT) {
		if (replyType === 'audio') audioReply(ctx, "Достигнут лимит сообщений. Лимит будет возобновлен после полуночи");
		else ctx.replyWithMarkdown(`You have reached the message limit of \`${LIMIT}\` messages. Please wait until midnight UTC to send more messages.`);
                return;
            }
            // Get the users message
            ...
            // If the message is empty, send a message to the user
            if (message.trim() === "") {
		if (replyType === 'audio') audioReply(ctx, "Отправьте не пустое сообщение! Скажите что-нибудь!");
		else ctx.replyWithMarkdown("Отправьте не пустое сообщение! Скажите что-нибудь!");
                return;
            }
            ...
            if (response === null) {
		if (replyType === 'audio') audioReply(ctx, "Не расслышал вас, повторите пожалуйста снова!");
		else ctx.replyWithMarkdown("Не расслышал вас, повторите пожалуйста снова!");                
                return;
            }
	    ...
	    if (replyType === 'audio') audioReply(ctx, reply);
...
function audioReply(ctx: any, messageText: string) {
		let ttsParams = new URLSearchParams();
		ttsParams.append('text',  messageText);
		ttsParams.append('folderId', process.env.YANDEX_API_FOLDER_ID as string);
...
function processAudioFile(ctx: any, file_id: string) {
